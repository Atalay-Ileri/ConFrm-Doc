...
%Nickolai: a good starting point might be to articulate what problem
%you are trying to solve and what your approach / design for
%addressing that problem looks like.  so, something along the lines of
%the goal and design sections.

%FK: it would be good to explain in intro or next section the
%challenge of nailing down a spec because it must limit an adversarial implementation.
...
%FK: it would good to explain why this example above satisfies NI
...
%FK utilization -> primitive (above)
...
%FK Is the last paragraph the general solution to handling unsafe primitives?
...
%FK: it would be great to have the examples; now I have a difficult
